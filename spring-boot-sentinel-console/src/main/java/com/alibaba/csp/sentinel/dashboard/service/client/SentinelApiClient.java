package com.alibaba.csp.sentinel.dashboard.service.client;

import com.alibaba.csp.sentinel.adapter.gateway.common.rule.GatewayFlowRule;
import com.alibaba.csp.sentinel.command.CommandConstants;
import com.alibaba.csp.sentinel.command.vo.NodeVo;
import com.alibaba.csp.sentinel.config.SentinelConfig;
import com.alibaba.csp.sentinel.dashboard.dao.entity.SentinelVersion;
...
import com.alibaba.csp.sentinel.dashboard.exception.CommandFailedException;
import com.alibaba.csp.sentinel.dashboard.exception.CommandNotFoundException;
import com.alibaba.csp.sentinel.dashboard.service.discovery.AppManagement;
import com.alibaba.csp.sentinel.dashboard.util.AsyncUtils;
import com.alibaba.csp.sentinel.dashboard.util.SentinelApiConstant;
...
import com.alibaba.csp.sentinel.dashboard.vo.cluster.state.ClusterServerStateVO;
...
public class SentinelApiClient {
    ...
    public List<NodeVo> fetchResourceOfMachine(String ip, int port, String type) {
        return fetchItems(ip, port, SentinelApiConstant.RESOURCE_URL_PATH, type, NodeVo.class);
    }
    /**
     * Fetch cluster node.
     *
    ...
    public List<NodeVo> fetchClusterNodeOfMachine(String ip, int port, boolean includeZero) {
        ...
        return fetchItems(ip, port, SentinelApiConstant.CLUSTER_NODE_PATH, type, NodeVo.class);
    }
    public List<FlowRuleEntity> fetchFlowRuleOfMachine(String app, String ip, int port) {
        List<FlowRule> rules = fetchRules(ip, port, SentinelApiConstant.FLOW_RULE_TYPE, FlowRule.class);
        if (rules != null) {
    ...
    public List<DegradeRuleEntity> fetchDegradeRuleOfMachine(String app, String ip, int port) {
        List<DegradeRule> rules = fetchRules(ip, port, SentinelApiConstant.DEGRADE_RULE_TYPE, DegradeRule.class);
        if (rules != null) {
    ...
    public List<SystemRuleEntity> fetchSystemRuleOfMachine(String app, String ip, int port) {
        List<SystemRule> rules = fetchRules(ip, port, SentinelApiConstant.SYSTEM_RULE_TYPE, SystemRule.class);
        if (rules != null) {
    ...
    public CompletableFuture<List<ParamFlowRuleEntity>> fetchParamFlowRulesOfMachine(String app, String ip, int port) {
        try {
            ...
            return fetchItemsAsync(ip, port, SentinelApiConstant.GET_PARAM_RULE_PATH, null, ParamFlowRule.class)
    ...
    public List<AuthorityRuleEntity> fetchAuthorityRulesOfMachine(String app, String ip, int port) {
        ...
        params.put("type", SentinelApiConstant.AUTHORITY_TYPE);
        List<AuthorityRule> rules = fetchRules(ip, port, SentinelApiConstant.AUTHORITY_TYPE, AuthorityRule.class);
    ...
    public boolean setFlowRuleOfMachine(String app, String ip, int port, List<FlowRuleEntity> rules) {
        return setRules(app, ip, port, SentinelApiConstant.FLOW_RULE_TYPE, rules);
    }
    ...
    public boolean setDegradeRuleOfMachine(String app, String ip, int port, List<DegradeRuleEntity> rules) {
        return setRules(app, ip, port, SentinelApiConstant.DEGRADE_RULE_TYPE, rules);
    }
    ...
    public boolean setSystemRuleOfMachine(String app, String ip, int port, List<SystemRuleEntity> rules) {
        return setRules(app, ip, port, SentinelApiConstant.SYSTEM_RULE_TYPE, rules);
    }
    public boolean setAuthorityRuleOfMachine(String app, String ip, int port, List<AuthorityRuleEntity> rules) {
        return setRules(app, ip, port, SentinelApiConstant.AUTHORITY_TYPE, rules);
    }
    ...
    public CompletableFuture<Void> setParamFlowRuleOfMachine(String app, String ip, int port, List<ParamFlowRuleEntity> rules) {
        ...
        try {
            ...
            return executeCommand(app, ip, port, SentinelApiConstant.SET_PARAM_RULE_PATH, params, true)
                    .thenCompose(e -> {
    ...
    public CompletableFuture<ClusterStateSimpleEntity> fetchClusterMode(String ip, int port) {
        ...
        try {
            return executeCommand(ip, port, SentinelApiConstant.FETCH_CLUSTER_MODE_PATH, false)
    ...
    public CompletableFuture<Void> modifyClusterMode(String ip, int port, int mode) {
        ...
        try {
            ...
            return executeCommand(ip, port, SentinelApiConstant.MODIFY_CLUSTER_MODE_PATH, params, false)
                    .thenCompose(e -> {
                        if (CommandConstants.MSG_SUCCESS.equals(e)) {
                            return CompletableFuture.completedFuture(null);
                        } else {
    ...
    public CompletableFuture<ClusterClientInfoVO> fetchClusterClientInfoAndConfig(String ip, int port) {
        ...
        try {
            return executeCommand(ip, port, SentinelApiConstant.FETCH_CLUSTER_CLIENT_CONFIG_PATH, false)
    ...
    public CompletableFuture<Void> modifyClusterClientConfig(String app, String ip, int port, ClusterClientConfig config) {
        ...
        try {
            ...
            return executeCommand(app, ip, port, SentinelApiConstant.MODIFY_CLUSTER_CLIENT_CONFIG_PATH, params, true)
                    .thenCompose(e -> {
                        if (CommandConstants.MSG_SUCCESS.equals(e)) {
                            return CompletableFuture.completedFuture(null);
                        } else {
    ...
    public CompletableFuture<Void> modifyClusterServerFlowConfig(String app, String ip, int port, ServerFlowConfig config) {
        ...
        try {
            Map<String, String> params = new HashMap<>(1);
            params.put("data", JSON.toJSONString(config));
            return executeCommand(app, ip, port, SentinelApiConstant.MODIFY_CLUSTER_SERVER_FLOW_CONFIG_PATH, params, true)
                    .thenCompose(e -> {
                        if (CommandConstants.MSG_SUCCESS.equals(e)) {
                            return CompletableFuture.completedFuture(null);
                        } else {
    ...
    public CompletableFuture<Void> modifyClusterServerTransportConfig(String app, String ip, int port, ServerTransportConfig config) {
        ...
        try {
            ...
            return executeCommand(app, ip, port, SentinelApiConstant.MODIFY_CLUSTER_SERVER_TRANSPORT_CONFIG_PATH, params, false)
                    .thenCompose(e -> {
                        if (CommandConstants.MSG_SUCCESS.equals(e)) {
                            return CompletableFuture.completedFuture(null);
                        } else {
    ...
    public CompletableFuture<Void> modifyClusterServerNamespaceSet(String app, String ip, int port, Set<String> set) {
        ...
        try {
            ...
            return executeCommand(app, ip, port, SentinelApiConstant.MODIFY_CLUSTER_SERVER_NAMESPACE_SET_PATH, params, true)
                    .thenCompose(e -> {
                        if (CommandConstants.MSG_SUCCESS.equals(e)) {
                            return CompletableFuture.completedFuture(null);
                        } else {
    ...
    public CompletableFuture<ClusterServerStateVO> fetchClusterServerBasicInfo(String ip, int port) {
        ...
        try {
            return executeCommand(ip, port, SentinelApiConstant.FETCH_CLUSTER_SERVER_BASIC_INFO_PATH, false)
    ...
    public CompletableFuture<List<ApiDefinitionEntity>> fetchApis(String app, String ip, int port) {
        ...
        try {
            return executeCommand(ip, port, SentinelApiConstant.FETCH_GATEWAY_API_PATH, false)
                    .thenApply(r -> {
    ...
    public boolean modifyApis(String app, String ip, int port, List<ApiDefinitionEntity> apis) {
        ...
        try {
            ...
            Map<String, String> params = new HashMap<>(2);
            params.put("data", data);
            String result = executeCommand(app, ip, port, SentinelApiConstant.MODIFY_GATEWAY_API_PATH, params, true).get();
    ...
    public CompletableFuture<List<GatewayFlowRuleEntity>> fetchGatewayFlowRules(String app, String ip, int port) {
        ...
        try {
            return executeCommand(ip, port, SentinelApiConstant.FETCH_GATEWAY_FLOW_RULE_PATH, false)
                    .thenApply(r -> {
    ...
    public boolean modifyGatewayFlowRules(String app, String ip, int port, List<GatewayFlowRuleEntity> rules) {
        ...
        try {
            ...
            Map<String, String> params = new HashMap<>(2);
            params.put("data", data);
            String result = executeCommand(app, ip, port, SentinelApiConstant.MODIFY_GATEWAY_FLOW_RULE_PATH, params, true).get();
    ...
    private <T extends Rule> List<T> fetchRules(String ip, int port, String type, Class<T> ruleType) {
        return fetchItems(ip, port, SentinelApiConstant.GET_RULES_PATH, type, ruleType);
    }
    private boolean setRules(String app, String ip, int port, String type, List<? extends RuleEntity> entities) {
        ...
        try {
            ...
            params.put("type", type);
            params.put("data", data);
            String result = executeCommand(app, ip, port, SentinelApiConstant.SET_RULES_PATH, params, true).get();
            log.info("setRules: {}", result);
            return true;
        } catch (InterruptedException | ExecutionException e) {
            log.warn("setRules api failed: {}", type, e);
            return false;
        } catch (Exception e) {
            log.warn("setRules failed", e);
            return false;
        }
    }
    ...
    private HttpUriRequest postRequest(String url, Map<String, String> params) {
        ...
        if (params != null && params.size() > 0) {
            ...
            try {
                ...
                log.warn("httpPostContent encode entity error: {}", params, e);
                return null;
            }
        }
        return httpPost;
    ...
    private String urlEncode(String str) {
        try {
            ...
            log.info("encode string error: {}", str, e);
            return null;
        }
    }
