package com.ai.spring.boot.content.conf.sentinel.file;
...
public class FileDataSourceInit implements InitFunc {
    ...
    public void init() throws Exception {
        if (ruleDir == null){
            ruleDir = SentinelConfUtil.getSentinelConfUtil().getSentinelConf().getBasePath();
        }
        ...
        ReadableDataSource<String,List<FlowRule>> flowRuleRDS =
                new FileRefreshableDataSource<List<FlowRule>>(flowRulePath,flowRuleListParser);
        // 将可读数据源注册到FlowRuleManager
        FlowRuleManager.register2Property(flowRuleRDS.getProperty());

        WritableDataSource<List<FlowRule>> flowRuleWDS =
                new FileWritableDataSource<List<FlowRule>>(flowRulePath,this::encodeJosn);
        // 将可写数据源注册到transport模块的WriteableDataSourceRegistry中
        // 这样收到控制台推送的规则时，sentinel会先更新到内存，然后将规则写入到文件中。
        WritableDataSourceRegistry.registerFlowDataSource(flowRuleWDS);

        // 降级规则
        ReadableDataSource<String,List<DegradeRule>> degradeRuleRDS =
                new FileRefreshableDataSource<List<DegradeRule>>(degradeRulePath,degradeRuleListParser);
        DegradeRuleManager.register2Property(degradeRuleRDS.getProperty());
        WritableDataSource<List<DegradeRule>> degradeRuleWDS =
                new FileWritableDataSource<List<DegradeRule>>(degradeRulePath,this::encodeJosn);
        WritableDataSourceRegistry.registerDegradeDataSource(degradeRuleWDS);

        // 系统规则
        ReadableDataSource<String,List<SystemRule>> systemRuleRDS =
                new FileRefreshableDataSource<List<SystemRule>>(systemRulePath,systemRuleListParser);
        SystemRuleManager.register2Property(systemRuleRDS.getProperty());
        WritableDataSource<List<SystemRule>> systemRuleWDS =
                new FileWritableDataSource<List<SystemRule>>(systemRulePath,this::encodeJosn);
        WritableDataSourceRegistry.registerSystemDataSource(systemRuleWDS);

        // 授权规则
        ReadableDataSource<String,List<AuthorityRule>> authorityRuleRDS =
                new FileRefreshableDataSource<List<AuthorityRule>>(authorityRulePath,authorityRuleListParser);
        AuthorityRuleManager.register2Property(authorityRuleRDS.getProperty());
        WritableDataSource<List<AuthorityRule>> authorityRuleWDS =
                new FileWritableDataSource<List<AuthorityRule>>(authorityRulePath,this::encodeJosn);
        WritableDataSourceRegistry.registerAuthorityDataSource(authorityRuleWDS);

        // 热点参数规则
        ReadableDataSource<String,List<ParamFlowRule>> paramFlowRuleRDS =
                new FileRefreshableDataSource<List<ParamFlowRule>>(paramFlowRulePath,paramFlowRuleListParser);
        ParamFlowRuleManager.register2Property(paramFlowRuleRDS.getProperty());
        WritableDataSource<List<ParamFlowRule>> paramFlowRuleWDS =
                new FileWritableDataSource<List<ParamFlowRule>>(authorityRulePath,this::encodeJosn);
